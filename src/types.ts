...
export type Async<T extends object> = {
  [k in Keys<T>]: T[k] extends Callable
    ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ReturnType<T[k] extends Callable ? T[k] : never> extends Promise<any>
      ? T[k]
      : (...args: Parameters<T[k]>) => Promise<ReturnType<T[k] extends Callable ? T[k] : never>>
    : T[k] extends object
      ? Async<T[k]>
      : never;
};
